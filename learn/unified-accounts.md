---
title: 统一账户
description: 目前Thales本地使用基于以太坊的H160账户系统且获得Polkadot JS Apps的支持。
---
# 统一账户
...
随着[Thalesbase Alpha测试网v3升级版本的发布](https://www.purestake.com/news/thales-network-upgrades-account-structure-to-match-ethereum/)，我们已对Thales底层账户系统进行重大升级，将默认的Substrate式账户和密钥替换为以太坊式账户和密钥。

同时我们也升级了Polkadot JS Apps交互界面，以实现对H160地址和ECDSA密钥的原生支持。您可参考[此教程](/integrations/wallets/polkadotjs/)获取更多信息。

## Substrate EVM兼容的区块链

波卡（Polkadot）生态系统中所有平行链都可实现完全兼容EVM，让Solidity智能合约仅需稍作修改甚至不需要修改即可执行。Substrate让这一集成成为可能——只需将[EVM模块](https://docs.rs/pallet-evm/2.0.1/pallet_evm/)插入运行时间，以获取EVM支持，并且插入[Ethereum Pallet with Frontier](https://github.com/paritytech/frontier)获得以太坊RPC兼容性。Thales和Parity共同开发了这些开源模块，让许多平行链能够兼容以太坊。
...
Thales致力于在Polkadot上创造一个完全兼容以太坊的环境，并提供最好的用户体验。除了基本的以太坊功能以外，还提供了链上治理、质押挖矿、跨链整合等额外功能。
...
可以看到，Bob仅有一个配对地址的私钥。他不需要在两个不同账户之间转移余额，只需通过一个账户和一个私钥就可以获取所有功能。Thales对单一账户进行了标准化调整，以符合以太坊式H160地址和ECDSA密钥标准。